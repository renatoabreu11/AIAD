	...
	public void step() throws Exception {
		...
		case 1: {
			...
			if (!this.route.atDestination()) {
				...
				if (this.goingHome) {
					this.goingHome = false;
					Building R = ContextManager.buildingContext.getRandomObject();
					this.route = new Route(R.getCoords(), R.getCoords()); //TODO refactor primeiro parâmetro é falso
				} 
				}
		...
		case 5: {
			...
			if (this.route == null) {
				...
				Building R = ContextManager.buildingContext.getRandomObject();
				while (R == home) {
					R = ContextManager.buildingContext.getRandomObject();
				}
				
				this.route = new Route(this, ContextManager.buildingProjection.getGeometry(R).getCentroid().getCoordinate(), R);
				this.origin = ContextManager.getAgentGeometry(this).getCoordinate();
				this.destination = ContextManager.buildingProjection.getGeometry(R).getCentroid().getCoordinate();
			}
